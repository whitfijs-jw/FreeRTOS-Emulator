...
#define PADDLE_INCREMENT_COUNT (GAME_FIELD_HEIGHT_INNER - PADDLE_LENGTH) / PADDLE_INCREMENT_SIZE
...
void vDrawWall(wall_t* wall) { checkDraw(tumDrawFilledBox(wall->x1, wall->y1, wall->w, wall->h, wall->colour), __FUNCTION__); }

void vDrawPaddle(wall_t* wall, unsigned short y_increment)
{
    // Set wall Y
    setWallProperty(wall, 0, y_increment * PADDLE_INCREMENT_SIZE + GAME_FIELD_INNER + 2, 0, 0, SET_WALL_Y);
    // Draw wall
    vDrawWall(wall);
}
...
void vDrawScores(unsigned int left, unsigned int right)
{
    ...
    checkDraw(tumDrawText(buffer, SCREEN_WIDTH / 2 - size - SCORE_CENTER_OFFSET, SCORE_TOP_OFFSET, White), __FUNCTION__);
    sprintf(buffer, "%d", left);
    checkDraw(tumDrawText(buffer, SCREEN_WIDTH / 2 + SCORE_CENTER_OFFSET, SCORE_TOP_OFFSET, White), __FUNCTION__);
}
...
void vResetPaddle(wall_t* wall) { setWallProperty(wall, 0, PADDLE_INCREMENT_COUNT / 2, 0, 0, SET_WALL_Y); }
...
void vRightPaddleTask(void* pvParameters)
{
    ...
    // Right wall
    wall_t* right_wall = createWall(GAME_FIELD_INNER + GAME_FIELD_WIDTH_INNER, GAME_FIELD_OUTER, WALL_THICKNESS, GAME_FIELD_HEIGHT_OUTER, 0.1, White, &vRightWallCallback, &right_player);
    // Right paddle
    right_player.paddle = createWall(SCREEN_WIDTH - PADDLE_EDGE_OFFSET - PADDLE_WIDTH - GAME_FIELD_INNER, PADDLE_START_LOCATION_Y, PADDLE_WIDTH, PADDLE_LENGTH, 0.1, White, NULL, NULL);
...
void vLeftPaddleTask(void* pvParameters)
{
    ...
    // Left wall
    wall_t* left_wall = createWall(GAME_FIELD_OUTER, GAME_FIELD_OUTER, WALL_THICKNESS, GAME_FIELD_HEIGHT_OUTER, 0.1, White, &vLeftWallCallback, &left_player);
    // Left paddle
    left_player.paddle = createWall(GAME_FIELD_INNER + PADDLE_EDGE_OFFSET, PADDLE_START_LOCATION_Y, PADDLE_WIDTH, PADDLE_LENGTH, 0.1, White, NULL, NULL);
...
void vPongControlTask(void* pvParameters)
{
    ...
    ball_t* my_ball = createBall(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, White, 6, 1000, &playBallSound, NULL);
    ...
    // Top wall
    wall_t* top_wall = createWall(GAME_FIELD_INNER, GAME_FIELD_OUTER, GAME_FIELD_WIDTH_INNER, WALL_THICKNESS, 0.1, White, NULL, NULL);
    // Bottom wall
    wall_t* bottom_wall = createWall(GAME_FIELD_INNER, GAME_FIELD_INNER + GAME_FIELD_HEIGHT_INNER, GAME_FIELD_WIDTH_INNER, WALL_THICKNESS, 0.1, White, NULL, NULL);
    ...
    while (1) {
        if (xSemaphoreTake(DrawReady, portMAX_DELAY) == pdTRUE) {
            ...
            for (int i = 0; i < NET_DOTS; i++)
                checkDraw(tumDrawFilledBox(SCREEN_WIDTH / 2 - NET_DOT_WIDTH / 2, GAME_FIELD_INNER + round(2.0 * i * NET_DOT_HEIGHT), NET_DOT_WIDTH, round(NET_DOT_HEIGHT), White), __FUNCTION__);
...
void vPausedStateTask(void* pvParameters)
{
    ...
    while (1) {
        ...
        checkDraw(tumDrawText((char*)paused_text, SCREEN_WIDTH / 2 - text_width / 2, SCREEN_HEIGHT / 2, Red), __FUNCTION__);
...
int main(int argc, char* argv[])
{
    ...
    xTaskCreate(vLeftPaddleTask, "LeftPaddleTask", mainGENERIC_STACK_SIZE, NULL, mainGENERIC_PRIORITY, &LeftPaddleTask);
    xTaskCreate(vRightPaddleTask, "RightPaddleTask", mainGENERIC_STACK_SIZE, NULL, mainGENERIC_PRIORITY, &RightPaddleTask);
    xTaskCreate(vPausedStateTask, "PausedStateTask", mainGENERIC_STACK_SIZE, NULL, mainGENERIC_PRIORITY, &PausedStateTask);
    xTaskCreate(vPongControlTask, "PongControlTask", mainGENERIC_STACK_SIZE, NULL, mainGENERIC_PRIORITY, &PongControlTask);
